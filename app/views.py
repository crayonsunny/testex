...
from forms import LoginForm, RegistrationForm, RenewalForm, DeleteForm
...
def index():
    if form.validate_on_submit():
         ...
         if file and allowed_file(file.filename):
             ...
             file.save(os.path.join(UPLOAD_FOLDER, log))
             return redirect(url_for("renew"))
    user = g.user     #{ 'nickname': 'Miguel' } # выдуманный пользователь
    files = [ # список выдуманных постов
        { 
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user:
            #v bd est' user s takimi loginom i parolem
            # avtorizuem i delaem redirect
...
def reg():
    ...
    #esli ne poluchitsa sdelat' validator, to otsledit' zdes' povtorenie logina i pochty
       log = form.login.data
       eml = form.email.data
